<script lang="ts">
    import type { Alert } from "$lib/components/Alerts/Alerts";
    ...
    async function dragEndHandler(e: MouseEvent) {
        ...
        if (should_delete()) {
            dispatch("delete");
            flyOutDirection = dragOffset.x > 0 ? 1 : -1;
            requestAnimationFrame(fly_out);
        } else {
    ...
    function fly_out() {
        if (flyOutDirection === 0) return;
        const speed = 10;
        dragOffset.x += speed * flyOutDirection;
        elem.style.setProperty("--x", `${dragOffset.x}px`);
        if (
            (flyOutDirection === 1 && dragOffset.x > elem.clientWidth) ||
            (flyOutDirection === -1 && dragOffset.x < -elem.clientWidth)
        ) {
            dispatch("delete");
        } else {
